...
const ProfileCreation: React.FC<Props> = ({ onProfileComplete, onCancel }) => {
  ...
  // Get current user from localStorage with debug logs
  useEffect(() => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!currentUser || !currentUser._id) {
      ...
      console.error('❌ No current user found with ID');
      return;
    }

    setIsSaving(true);

    // DECLARE profile HERE - FIXED
    const profile = {
      ...formData,
      techStack,
      selectedProject
    ...
    try {
      console.log('🚀 Starting profile creation for user:', currentUser.name);
      ...
      if (onProfileComplete) onProfileComplete(profile);
